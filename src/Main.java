...
public class Main{
    ...
    // table for Main cipher
    private String [][] table;

    // main method to test Main method
    public static void main(String[] args){
        Main pf = new Main();
    }

    // main run of the program, Main method
    private Main(){

        // prompts user for the keyword to use for encoding & creates tables
        Scanner sc = new Scanner(System.in);
        int choice;
        System.out.println("1. for encode \n 2. to decode \n 3. to exit");
        choice=sc.nextInt();
        while( choice !=3) {
            System.out.println("Please input the keyword for the Main cipher.");
            ...
            switch (choice) {
                case 1: {
                    ...
                    this.printResults(decodedOutput);
                    System.out.println();
                    System.out.println("after Removing 'X' = "+decodedOutput.replace("X", ""));
                    System.out.println();
                    break;
                }
                ...
                case 2: {
                    ...
                    String todecode = parseString(sc);
                    while (todecode.equals(""))
                        todecode = parseString(sc);
                    System.out.println();

                    String decodedOutput = decode(todecode);

                    // output the results to user
                    this.printTable(table);
                    this.printResultsde(decodedOutput);
                    System.out.println("after Removing 'X' = "+decodedOutput.replace("X", ""));
                    System.out.println();
                    break;
                }
    ...
    private String[][] cipherTable(String key){
        ...
        for(int k = 0; k < keyString.length(); k++){
            ...
            for(int i = 0; i < 5; i++){
                for(int j = 0; j < 5; j++){
                    if(MainTable[i][j].equals("" + keyString.charAt(k))){
                        repeat = true;
                    }else if(MainTable[i][j].equals("") && !repeat && !used){
                        MainTable[i][j] = "" + keyString.charAt(k);
                        used = true;
                    }
                }
            }
    ...
    private void printResults(String pri){
        System.out.println("This is the encoded message:");
        System.out.println(pri);
        System.out.println();

    }
    private void printResultsde(String pri){
